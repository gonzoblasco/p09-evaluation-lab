...
export async function GET(
  ...
  try {
    response = await fetch(`${mcpUrl}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        id: 1,
        method: 'tools/call',
        params: { name: 'get_executions', arguments: { workflowId, limit: 10 } },
      }),
    })
  } catch (err) {
  ...
  try {
    executions = JSON.parse(result)
  } catch {
    return NextResponse.json({ error: 'Failed to parse executions JSON' }, { status: 502 })
  }
